...
class TicketModal(ui.Modal, title="Verify your Ticket"):
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        ...
        organizer_role = discord.utils.get(interaction.guild.roles, name=config.ORGANIZER_ROLE_NAME)
        if not organizer_role:
            self.success = False
            await interaction.response.send_message(messages.TICKET_GENERIC_ERROR_MESSAGE.format(role=f"@{config.ORGANIZER_ROLE_NAME}"))
        ...
        try:
            ...
            self.success = False
            logger.error(f"Error claiming ticket for {interaction.user.name} ({interaction.user.id}): {e}")
            return
        except exceptions.InvalidTicketIdException as e:
            self.success = False
            await interaction.response.send_message(messages.TICKET_INVALID_ID_MESSAGE, ephemeral=True, delete_after=10)
            return
        except exceptions.TicketHolderRoleAlreadyAssignedException as e:
            self.success = False
            await interaction.response.send_message(messages.TICKET_MEMBER_ALREADY_CLAIMED_MESSAGE, ephemeral=True, delete_after=10)
            return
        except exceptions.MemberHasNotReactedToCocException as e:
            self.success = False
            await interaction.response.send_message(messages.COC_NOT_ACCEPTED_MESSAGE, ephemeral=True, delete_after=10)
            return
        except exceptions.TicketAlreadyClaimedException as e:
            self.success = False
            await interaction.channel.add_user(random_organizer)
            await interaction.response.send_message(messages.TICKET_MEMBER_ALREADY_CLAIMED_WITH_NO_ROLE_MESSAGE.format(role=organizer_role.mention))
            return
        except exceptions.TicketNotFoundInDatabaseException as e:
            self.success = False
            await interaction.channel.add_user(random_organizer)
            await interaction.response.send_message(messages.TICKET_NOT_FOUND_IN_DATABASE_MESSAGE.format(role=organizer_role.mention), ephemeral=True, delete_after=10)
            return
        except Exception as e:
            self.success = False
            await interaction.response.send_message(messages.TICKET_DB_ERROR_MESSAGE.format(role=organizer_role.mention))
            logger.error(f"Error claiming ticket for {interaction.user.name} ({interaction.user.id}): {e}")
            return
        ...
        if not member_can_claim_ticket:
            self.success = False
            await interaction.channel.add_user(random_organizer)
            await interaction.response.send_message(messages.TICKET_GENERIC_ERROR_MESSAGE.format(role=organizer_role.mention))
            logger.error(f"Error claiming ticket for {interaction.user.name} ({interaction.user.id})")
            return
        ...
        try:
            ...
            self.success = False
            await interaction.channel.add_user(random_organizer)
            await interaction.response.send_message(messages.TICKET_ROLE_ASSIGNMENT_ERROR_MESSAGE.format(role=organizer_role.mention))
            return
        except Exception as e:
            self.success = False
            await interaction.channel.add_user(random_organizer)
