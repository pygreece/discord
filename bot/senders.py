...
async def send_direct_message(
        member: discord.Member, content: str
    ) -> bool:
...
async def send_message_in_channel(
    ...
    if f"{purpose_prefix}-{MEMBER_ROLE_NAME}" not in [category.name for category in guild.categories]:
        category = await guild.create_category(f"{purpose_prefix}-{MEMBER_ROLE_NAME}", reason="Fallback for closed DMs")
        if not category:
    ...
    channel = await category.create_text_channel(
        name=f"{purpose_prefix}-{member.name}", overwrites=overwrites, reason="Fallback for closed DMs"
    )
    if not channel:
