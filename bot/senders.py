    ...
    if not category:
        try:
            category = await guild.create_category(
                name=category_name,
                reason=f"{category_name} for {channel_prefix}-member_name channels",
            )
...
async def delete_channel_and_or_category(
    member: discord.Member, channel_prefix: str, category_name: str, reason: str
) -> None:
    """
    ...
    if channel:
        try:
            ...
            logger.info(
                f"Deleted private {channel_prefix}-{member_name} channel for {member.name} ({member.id}) because {reason}."
            )
        except Exception as e:
